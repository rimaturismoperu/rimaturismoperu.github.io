/* Rimaturismo Perú · Worker autónomo generado automáticamente.
...
function buildIndex(indexTemplate, rawState) {
...
function buildTour(tourInput, rawState) {
  const state = normalizeState(rawState);
...
function buildLegalPage(kind, rawState) {
...
function buildSitemap(rawState) {
  const state = normalizeState(rawState);
  const urls = [
    `${state.site.baseUrl}/`,
...
function buildRobots(rawState) {
  const { site } = normalizeState(rawState);
  return `User-agent: *\nAllow: /\nDisallow: /admin/\nDisallow: /setup/\nDisallow: /data/\nDisallow: /lib/\nDisallow: /scripts/\nDisallow: /worker/\nSitemap: ${site.baseUrl}/sitemap.xml\n`;
}

function buildAll(indexTemplate, rawState) {
  const state = normalizeState(rawState);
  const files = {
